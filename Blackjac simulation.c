...
void generadorDeCartas(int valoresAleatorios[], int categoriasAleatorias[]) {
	...
	for(int i= 0; i<15; i++){
		valoresAleatorios[i] = 1 + rand()%13;
		categoriasAleatorias[i] = 1 + rand()%4;
	}
	...
	for(int i= 0; i<15; i++){
		for(int j=0; j<15; j++){
			if((valoresAleatorios[j] == valoresAleatorios[i]) && (i!=j) && (categoriasAleatorias[j] == categoriasAleatorias[i])){
				...
				valoresAleatorios[j] = 1 + rand()%13;
				categoriasAleatorias[j] = 1 + rand()%4;
...
int main(int argc, char *argv[]) {
	...
	while (fichas > 49){
		...
		//En caso de aceptar entrar a la partida
			...
			j=0;
			i=0;
			//puntaje_cartas_salidas[15];
			//puntaje_categoria_salidas[15];
